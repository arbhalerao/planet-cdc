...
const COMPAT_STYLES: Record<string, string> = {
  ...
  incompatible: "bg-red-900/60 text-red-400 border-red-900",
};
    ...
    <span className={`relative group inline-flex items-center gap-1 text-xs px-1.5 py-0.5 rounded border ${cls}`}>
      ...
      {reasons.length > 0 && (
        <>
          ...
          <span className="pointer-events-none absolute left-0 top-full mt-1 z-10 w-60 rounded bg-gray-950 border border-gray-700 px-2 py-1.5 text-xs text-gray-300 shadow-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-normal">
    ...
    <table className="w-full text-xs mt-2">
      ...
      <tbody className="divide-y divide-gray-800/60">
        {Object.entries(model.default_thresholds).map(([name, t]) => {
          ...
          return (
            <tr key={name}>
              <td className="py-1.5 font-mono text-gray-300 whitespace-nowrap">
                {name}
                {isPrimary && (
                  <span className="ml-1.5 text-xs bg-gray-700 text-gray-400 px-1 py-0.5 rounded">primary</span>
                )}
              </td>
              <td className="py-1.5 text-gray-400">{out?.description ?? "—"}</td>
              <td className="py-1.5 text-gray-500">{out?.unit || "—"}</td>
              <td className="py-1.5 text-gray-500">
                {out ? `${out.value_range[0]} – ${out.value_range[1]}` : "—"}
              </td>
              <td className="py-1.5 text-gray-400">{t.green[0]} – {t.green[1]}</td>
              <td className="py-1.5 text-gray-400">{t.yellow[0]} – {t.yellow[1]}</td>
              <td className="py-1.5 text-gray-400">{t.red[0]} – {t.red[1]}</td>
            </tr>
          );
        })}
      </tbody>
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-lg overflow-hidden">
      ...
      <div className="border-t border-gray-800">
        <button
          onClick={() => setOpen((o) => !o)}
          className="w-full px-5 py-2.5 flex items-center justify-between text-xs text-gray-400 hover:text-gray-200 hover:bg-gray-800/40 transition-colors"
        >
